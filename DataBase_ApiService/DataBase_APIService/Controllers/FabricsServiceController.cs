...
{
    public class FabricsServiceController : ApiController
    {
        [HttpGet]
        public IHttpActionResult Get()
        {
            try
            {
                ...
                {
                    List<FabricsModel> modelList = new GarmentsHandler().GetFabrics().ToModelList();
                    return Ok(modelList);
                }
                if (data.Count() == 1)
                {
                    switch (data.First().Key)
                    {
                        case "id":
                            ...
                            FabricsModel s = new GarmentsHandler().GetFabric(Id).ToModel();
                            return Ok(s);
