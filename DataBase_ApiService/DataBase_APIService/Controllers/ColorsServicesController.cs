...
{
    public class ColorsServicesController : ApiController
    {
        [HttpGet]
        public IHttpActionResult Get()
        {
            try
            {
                ...
                {
                    List<ColorsModel> modelList = new GarmentsHandler().GetColors().ToModelList();
                    return Ok(modelList);
                }
                if (data.Count() == 1)
                {
                    switch (data.First().Key)
                    {
                        case "id":
                            ...
                            return Ok(new GarmentsHandler().GetColor(Id).ToModel());
