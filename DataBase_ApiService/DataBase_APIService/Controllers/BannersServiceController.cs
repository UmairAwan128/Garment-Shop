...
{
    public class BannersServiceController : ApiController
    {
        public IHttpActionResult Get()
        {
            try
            {
                ...
                {
                    List<BannerModel> modelList = new GarmentsHandler().GetBanners().ToModelList();
                    return Ok(modelList);
                }
                if (data.Count() == 1)
                {
                    switch (data.First().Key)
                    {
                        case "id":
                            ...
                            return Ok(new GarmentsHandler().GetBanner(Id).ToModel());
